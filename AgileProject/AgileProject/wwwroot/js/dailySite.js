...
function onChangeExaminar() {
    ...
    if (file) {
        ...
        reader.onload = function (event) {
            ...
            document.getElementById("containerExample").style.backgroundImage = 'url(' + event.target.result + ')';
            //document.body.style.backgroundImage = 'url(' + event.target.result + ')'; //MFG
...
function loadPage() {
    ...
    //disable page back history.pushState(null, document.title, location.href);
    ...
    for (x of squads.equipos) {
        ...
        $('#myteamsList').append(newOption).trigger('change');
        //console.log($("#myteamsList").val())
    }
    */
}
...
function iniciar() {
    ...
    for (var i = 0; i < squads.equipos.length; i++) {
        if (text == squads.equipos[i].nombre) {
            //console.log(squads.equipos[i].nombre)
    ...
    if (nombres.length > 0) {
        ...
        if (objSelect !== null) {
            ...
            setTimeout(() => {
                ...
                document.getElementById("textoModal").innerHTML = "Winner!"
